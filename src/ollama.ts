...
export async function ollamaDocsLoader(options: {
  repo: string;
  path: string;
}): Promise<Loader> {
...
async function fetchDocsEntries(options: {
  ...
  const url = new URL(
    `https://api.github.com/repos/${options.repo}/contents/${options.path}`,
  );
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch ${options.repo}${options.path} from GitHub`,
    );
  }
